...
func (state *gameLiftServerState) init(params *ServerParameters, wsGameLift internal.IGameLiftManager) error {
	...
	state.processID = common.GetEnvStringOrDefault(common.GameliftSdkProcessID, params.ProcessID)
	state.hostID = common.GetEnvStringOrDefault(common.GameliftSdkHostID, params.HostID)
	state.fleetID = common.GetEnvStringOrDefault(common.GameliftSdkFleetID, params.FleetID)
	...
	err := state.wsGameLift.Connect(
		common.GetEnvStringOrDefault(common.GameliftSdkWebsocketURL, params.WebSocketURL),
		state.processID,
		state.hostID,
		state.fleetID,
		common.GetEnvStringOrDefault(common.GameliftSdkAuthToken, params.AuthToken),
	)
	if err != nil {
...
func (state *gameLiftServerState) getRoleCredentialsFromCache(roleArn string) (result.GetFleetRoleCredentialsResult, bool) {
	...
	if previousResult, ok := state.fleetRoleResultCache[roleArn]; ok {
		timeToLive := time.Duration(previousResult.Expiration-time.Now().Unix()) * time.Second
